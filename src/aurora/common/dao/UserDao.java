...
public class UserDao extends ANavigationDao implements IMasterDao {
	...
	public List<CatReg> getUsers(String loginId, String password) throws Exception{
		List<CatReg> list =null;
		//"select c.catTeamId as catTeamId,c.cat_name as cat_name,c.cat_email as cat_email,c.team_size as team_size,c.team_exp as team_exp,c.bar_arrg as bar_arrg,c.ser_style as ser_style,c.food_spec as food_spec,c.team_cat as team_cat,c.team_pwd as team_pwd,c.abt_cat as abt_cat where c.cat_email=loginId from CatReg c";
		String userQuery ="select u.catTeamId as catTeamId,u.cat_name as cat_name,u.team_pwd as team_pwd from CatReg u where u.cat_email='"+loginId+"'";			
		list = hibernateTemplate.getSessionFactory().getCurrentSession()
				
				.createQuery(userQuery)
				
				.setResultTransformer(Transformers.aliasToBean(CatReg.class))
				.list();
...
public List<?> getUserListForResetPwd(String lang) throws Exception {
	...
	//Event Oraganizers
	public Object addEmployee(Object employee) throws Exception {
		hibernateTemplate.clear();
		hibernateTemplate.flush();
	...
	public Object updateEmployee(Object employee) throws Exception {
		hibernateTemplate.clear();
		hibernateTemplate.flush();
	...
	public List<?> getAllEmployee(String lang) throws Exception {
	
		String userQuery = "";
		if (lang.equalsIgnoreCase("EN")) {
	
			userQuery = "select e.empId as empId,e.empName as empName,e.empUserName as empUserName,e.empPassword as empPassword,e.empAddress as empAddress,e.empPh as empPh,e.empDob as empDob FROM Employee e";
	
		} else {
	
			userQuery = "select e.EMP_ID as empId,e.EMP_NAME as empName,e.EMP_USERNAME as empUserName,e.EMP_PASSWORD as empPassword,e.EMP_ADDRESS as empAddress,e.EMP_PH as empPh,e.EMP_DOB as empDob FROM EMP_TABLE e";
	
		}
	
		List<Employee> list = hibernateTemplate.getSessionFactory()
				.getCurrentSession().createQuery(userQuery)
	...
	public Object deleteEmployee(long key) throws Exception {
		Employee obj;
		DetachedCriteria criteria = DetachedCriteria.forClass(Employee.class);
		criteria.add(Restrictions.eq("empId", key));
	
		List<Employee> gr =(List<Employee>) hibernateTemplate.findByCriteria(criteria);
		if (gr.size() > 0) {
			obj = gr.get(0);
