		...
		function initCss(){
			if (!$('#datagrid-filter-style').length){
				$('head').append(
					...
					'.datagrid-filter-row input{margin:0;-moz-border-radius:0;-webkit-border-radius:0;border-radius:0;background-color: #f5f50b;}' +
					'.datagrid-filter-c{overflow:hidden;color: black !important;}' +
		...
		function createFilter(frozen){
			...
			for(var i=0; i<fields.length; i++){
				...
				if (!div){
					...
					input.addClass('datagrid-filter').attr('name', field).attr('placeholder', 'Search here..').attr('autocomplete', 'off');
